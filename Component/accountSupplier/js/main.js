...
let moodBtn = "showSupplier";
function showContractor(){
    if(moodBtn != "showSupplier"){
        document.querySelector('.tableContractor').style.display = "block";
        document.querySelector('.tableSupplier').style.display = "none";
        document.querySelector('#btnShow').innerHTML = "الموردين";
        moodBtn = "showSupplier";

    }else{
        document.querySelector('.tableContractor').style.display = "none";
        document.querySelector('.tableSupplier').style.display = "block";
        document.querySelector('#btnShow').innerHTML = "المقاولين";
        moodBtn = "showContractor";
...
function showEmployees() {
    ...
    ipcRenderer.on('get-supplier',(e,args)=>{
        ...
        for(let i=0;i<client[client.length-1].length;i++){
            ...
            if(mE.mySupplier.typeSupplier == "مورد"){
                tableEmployees1 += `
                    <tr class="trOpen">
                        <td>${i+1}</td>
                        <td>${mE.mySupplier.code}</td>
                        <td>${mE.mySupplier.supplier}</td>
                        <td>${mE.mySupplier.newProject}</td>
                        <td>${mE.mySupplier.date}</td>
                        <td>${mE.mySupplier.address}</td>
                        <td>${mE.mySupplier.phone}</td>
                        <td>${mE.mySupplier.state}</td>
                        <td>${mE.mySupplier.notes}</td>
    ...
    for(let x=0; x<users.length;x++){
        if(userName.innerHTML == users[x].user && (users[x].job == 'it' || users[x].job == 'مدير')){
            ...
            ipcRenderer.on('get-client',(e,args)=>{
                ...
                tableEmployees = '';
                myEmployees.map(mE=>{
                    if(mE.mySupplier.state == "delete"){
                        tableEmployees += `
                        <tr>
                            <td>${mE.mySupplier.supplier}</td>
                            <td>${mE.mySupplier.nameProject}</td>
                            <td>${mE.mySupplier.date}</td>
                            <td>${mE.mySupplier.address}</td>
                            <td>${mE.mySupplier.phone}</td>
                            <td>${mE.mySupplier.notes}</td>
...
function searchData(value) {
    ...
    ipcRenderer.on('get-supplier',(e,args)=>{
        ...
        for(let i=0;i<client[client.length-1].length;i++){
            let mE = client[client.length-1][i];
            if(value != ""){
                if(mE.mySupplier.newProject.includes(value) ||  mE.mySupplier.supplier.includes(value) || mE.mySupplier.code.includes(value) ){
                    if(mE.mySupplier.state == "مفتوح"){
                        tableEmployees1 += `
                            <tr class="trOpen">
                                <td>${i+1}</td>
                                <td>${mE.mySupplier.code}</td>
                            ...
                            `
                    }else if(mE.mySupplier.state == "مغلق" ){
                        tableEmployees2 += `
                            <tr class="trClose">
                                <td>${i+1}</td>
                                <td>${mE.mySupplier.code}</td>
