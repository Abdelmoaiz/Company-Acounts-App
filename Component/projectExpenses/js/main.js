    ...
    for(let i=0;i<customer[customer.length-1].length;i++){
        ...
        // for(let x=0;x<customer[customer.length-1][i].myCustomer.details.length;x++){
            if(customer[customer.length-1][i].myCustomer.state == "مفتوح" && customer[customer.length-1][i].myCustomer.code ==  codeCustomer.value){
                ...
                item.focus();
                tmpIdCustomer = customer[customer.length-1][i]._id;
                tmpI = i;
                // tmpX = x;
...
createEmpl.addEventListener("click",(e)=>{
    ...
    if((nameClient.value && nameProject.value) != '') {
        ...
        if(mood == 'create') {
            ...
            customer[customer.length-1][tmpI].myCustomer.details.push(newClient);
            console.log(customer[customer.length-1][tmpI].myCustomer.details)
            let data = customer[customer.length-1][tmpI].myCustomer;
            
    
            ipcRenderer.send('update3-customer',{...data, tmpIdCustomer})
            ...
            customer[customer.length-1][tmpICustomerExp].myCustomer.details[tmpXCustomerExp] = newClient;
            // console.log(customer[customer.length-1][tmpI].myCustomer.details)
            let data = customer[customer.length-1][tmpICustomerExp].myCustomer;
            
    
            ipcRenderer.send('update-customerExpUpdate',{...data, tmpIdCustomerExp})
            window.location.reload();
...
function showEmployees() {
    ...
    ipcRenderer.on('get-customer',(e,args)=>{
        ...
        for(let i=0;i<customer[customer.length-1].length;i++){
            for(let x=0;x<customer[customer.length-1][i].myCustomer.details.length;x++){
                let data = customer[customer.length-1][i].myCustomer.details[x];
                if(customer[customer.length-1][i].myCustomer.details[x].state == "open" && customer[customer.length-1][i].myCustomer.details[x].typeAdd == "مصروف" ){
        ...
        if(userName.innerHTML == users[x].user && (users[x].job == 'it' || users[x].job == 'مدير')){
            ...
            for(let i=0;i<customer[customer.length-1].length; i++){
                if(customer[customer.length-1][i]._id == id){
                    let data = customer[customer.length-1][i].myCustomer.details[z];
...
function checkPass1(){
    ...
    if(document.querySelector("#inputPass").value == "100"){
        for(let i=0;i<customer[customer.length-1].length; i++){
            ...
            // console.log(customer[customer.length-1][i].myCustomer.details[tmpXDeleteExp])
            ...
            if(customer[customer.length-1][i]._id == tmpIdDeleteExp){
                ...
                // console.log(customer[customer.length-1][i].myCustomer.details[tmpXDeleteExp])
                customer[customer.length-1][i].myCustomer.details[tmpXDeleteExp].state = "delete";
                // console.log(customer[customer.length-1][i].myCustomer.details[tmpXDeleteExp])
                data = customer[customer.length-1][i].myCustomer;
                ipcRenderer.send('update-customerDeleteExp',{...data, tmpIdDeleteExp})
            ...
            ipcRenderer.on('get-customer',(e,args)=>{
                ...
                if(moodBtnShow == "delete"){
                    ...
                    for(let i=0;i<customer[customer.length-1].length;i++){
                        for(let x=0;x<customer[customer.length-1][i].myCustomer.details.length;x++){
                            let data = customer[customer.length-1][i].myCustomer.details[x];
                            if(customer[customer.length-1][i].myCustomer.details[x].state == "delete" && customer[customer.length-1][i].myCustomer.details[x].typeAdd == "مصروف" ){
                                // totalItem = +data.myExpenses.quantity * +data.myExpenses.priceExpenses;
                                tableEmployees += `
                                <tr>
                                    <td>${data.codeCustomer}</td>
...
function returnData(id,i,x){
    ...
    for(let i=0;i<customer[customer.length-1].length; i++){
        if(customer[customer.length-1][i]._id == id){
            ...
            customer[customer.length-1][i].myCustomer.details[x].state = "open";
            data = customer[customer.length-1][i].myCustomer;
            ipcRenderer.send('update-customerRestoreExp',{...data, id})
            refresh();
        }
...
function searchData(value) {
    ...
    ipcRenderer.on('get-customer',(e,args)=>{
        ...
        for(let i=0;i<customer[customer.length-1].length;i++){
            for(let x=0;x<customer[customer.length-1][i].myCustomer.details.length;x++){
                let data = customer[customer.length-1][i].myCustomer.details[x];
                if(customer[customer.length-1][i].myCustomer.state == "مفتوح" && data.typeAdd == "مصروف" &&(data.nameProject.includes(value) ||  data.nameCustomer.includes(value) || data.codeCustomer.includes(value))){
                    // totalItem = +data.myExpenses.quantity * +data.myExpenses.priceExpenses;
                    tableEmployees += `
                    <tr>
                        <td>${data.codeCustomer}</td>
...
function exportTableToExcel() {
    ...
    var wb = XLSX.utils.table_to_book(table, {sheet:"صرف المشاريع"});
    // write sheet to blob
    var blob = new Blob([s2ab(XLSX.write(wb, {bookType:'xlsx', type:'binary'}))], {
        type: "application/octet-stream"
    });
    // return sheet file
    return saveAs(blob, "صرف المشاريع.xlsx");
}
