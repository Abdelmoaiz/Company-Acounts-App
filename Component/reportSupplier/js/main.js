...
function getCustomer(value){
    ...
    ipcRenderer.on('get-supplier',(e,args)=>{
        ...
        for(let i=0;i<data.length;i++){
            if(codeCustomer.value == data[i].myClient.codeSupplier){
        ...
        // myEmployees.map(mE=>{
        //     if(mE.mySupplier.code ==  codeCustomer.value){
        ...
        if(search.value != ""){
            ...
            for(let i=0; i<customer[customer.length-1].length;i++){
                for(let x=0; x<customer[customer.length-1][i].mySupplier.details.length;x++){
                    if(customer[customer.length-1][i].mySupplier.details[x].state != "delete"){

                        if(customer[customer.length-1][i].mySupplier.details[x].date < searchFrom.value){
                            if(customer[customer.length-1][i].mySupplier.details[x].typeAdd == "توريد"){
                                totalSupply += +customer[customer.length-1][i].mySupplier.details[x].price;
                            }else{
                                totalExpenses += +customer[customer.length-1][i].mySupplier.details[x].price;
                            }

                            document.querySelector('.priceBack').innerHTML = +totalSupply - +totalExpenses;

                        }
                        if((customer[customer.length-1][i].mySupplier.code.includes(search.value) &&  customer[customer.length-1][i].mySupplier.details[x].date >= searchFrom.value && customer[customer.length-1][i].mySupplier.details[x].date <= searchTo.value) ){
                            // allData.push(client[client.length-1][i])
                            z= x

                            document.querySelector('.nameSupply').innerHTML = customer[customer.length-1][i].mySupplier.supplier;
                            document.querySelector('.nameProject').innerHTML = customer[customer.length-1][i].mySupplier.newProject;
                            ...
                            document.querySelector('.codeCustomer').innerHTML = customer[customer.length-1][i].mySupplier.code;
                            document.querySelector('.priceProject').innerHTML = 0;
                            ...
                            if(customer[customer.length-1][i].mySupplier.details[x].typeAdd == "توريد"){
                                ...
                                <tr>
                                    ...
                                    <td>${customer[customer.length-1][i].mySupplier.details[x].date}</td>
                                    
                                    <td>${customer[customer.length-1][i].mySupplier.details[x].title}</td>
                                    <td></td>
                                    <td></td>
                                    <td>${customer[customer.length-1][i].mySupplier.details[x].price}</td>
                                ...
                                <tr>
                                    ...
                                    <td>${customer[customer.length-1][i].mySupplier.details[x].title}</td>
                                    <td>${customer[customer.length-1][i].mySupplier.details[x].quantity}</td>
                                    <td>${customer[customer.length-1][i].mySupplier.details[x].price}</td>
                                    <td></td>
                                    <td>${+totalSupply - +totalExpenses}</td>
                ...
                // console.log(customer[customer.length-1][0].mySupplier.details)
